    ...
    function precast(spell)
            if spell then
                    if spell.mp_cost > player.mp then
                            cancel_spell()
                            send_command('wait 4.0;input /ma "Indi-Refresh" <me>; wait 5.0; input /heal on')
                            return
                    end
            end
            if spell.type == "Geomancy" and skilluprun then
            ...
            if spell.type == "Ninjutsu" then
                    ...
                    if not (player.inventory == tool or player.inventory == stool) then
                            ...
                            elseif player.inventory == stoolbag then
                                    ...
                                    skilluprun = false
                                    add_to_chat(123,"You Have No More Of The Tools Needed For Spell "..ninspells[nincount].."\nStoping Nin Skillup")
                            end
                    end
            end
    end
